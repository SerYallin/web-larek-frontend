...
- `get total()` - геттер общей стоимости продуктов в корзине. Используется для отображения общей цены продуктов в корзине.
...
- `items:update` - событие обновления списка продуктов. Вызывается при обновлении списка продуктов и вызывает перерисовку списка продуктов на странице, а также обновление коллекции продуктов.
- `items:click` - событие выбора продукта. данное событие вызывает открытие модального окна с деталями продукта.
- `busket:add` - событие возникает при добавлении продукта в корзину. Обновляет список продуктов в корзине, обновления данных по количеству продуктов, и общей цены.
- `busket:remove` - событие возникает при удалении продукта из корзины. Обновляет список продуктов в корзине, обновления данных по количеству продуктов, и общей цены.
- `busket:update` - событие возникает при добавлении либо при удалении продутов из корзины, вызывает перерисовку иконки с корзиной, а также самой корзины, если модальное окно с корзиной открытое.
- `busket:open` - событие возникает при нажатии на кнопку(иконку) открыть корзину. Изменяет статус модального окна корзины на активный, а также прорисовывает модальное окно корзины со списком продуктов.
- `order:open` - событие вызывается при нажатии на кнопку "Оформить". Открывает модальное окно с формой заказа и создает объект ордера.
- `order:update` - событие вызывается при заполнении данных в ордере либо при нажатии кнопки "Далее". Обновляет данные в ордере и перерисовывает форму заказа.
- `order:submit` - событие вызывается при нажатии на кнопку "Отправить". Отправляет запрос на сервер с данными ордера и обновляет статус ордера и ошибки если они приходят.
- `order:submited` - данное событие срабатывает при получении результата ордра и открывает модальное окно со статусом ордера. Согласно заданию у нас успешное выполнение ордера.
- `modal:close` - событие вызывается при нажатии на кнопку "Закрыть" в модальном окне либо за пределами модального окна. Закрывает модальное окно и изменяет его статус.
